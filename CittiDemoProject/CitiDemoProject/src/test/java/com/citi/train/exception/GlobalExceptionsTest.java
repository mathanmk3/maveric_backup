...
class GlobalExceptionsTest {
    ...
    @Test
     void testHandleNoHandlerFoundException() {
        ...
        ResponseEntity<ErrorDto> response = globalExceptions.handleNoHandlerFoundException(ex);
        assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
        ErrorDto errorDto = response.getBody();
        assertNotNull(errorDto);
        assertEquals("Page Not found", errorDto.getErrorMessgae());
        assertEquals("404", errorDto.getErrorCode());
    }
